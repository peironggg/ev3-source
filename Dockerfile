...
RUN /bin/bash -c 'cd /dev/shm && \
  ...
  wget http://cdn-fastly.deb.debian.org/debian/pool/main/c/c-ares/libc-ares2_1.10.0-2+deb8u2_armel.deb && \
  wget http://cdn-fastly.deb.debian.org/debian/pool/main/libv/libv8-3.14/libv8-3.14.5_3.14.5.8-8.1_armel.deb && \
  wget http://cdn-fastly.deb.debian.org/debian/pool/main/n/nodejs/nodejs_0.10.29~dfsg-2_armel.deb && \
...
COPY source3 source3alt /usr/local/bin/
RUN /bin/bash -c 'chmod a+x /usr/local/bin/source3{,alt}'
